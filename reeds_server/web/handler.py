...
from web.mail import (
    TestMail,
    AWS_SES_HTMLMail,
    UserPasswordResetHTMLMessage,
    UserSignUpInstructionsHTMLMessage,
    UserSignUpRejectionHTMLMessage,
    UserSignUpRequestNotificationHTMLMessage,
    UserWelcomeHTMLMessage,
    UserSimRunInitiationHTMLMessage,
    UserSimInQueueHTMLMessage,
    UserSimRunCompleteHTMLMessage,
    UserChangePasswordHTMLMessage,
)
from web.constants import (
    USER_RESET_EMAIL_SUBJECT,
    USER_SIGNUP_EMAIL_SUBJECT,
    USER_SIGNUP_EMAIL_REJECT_SUBJECT,
    USER_SIGNUP_REQUEST_SUBJECT,
    USER_WELCOME_EMAIL_SUBJECT,
    REEDS_SIM_INITIATION_SUBJECT,
    REL_URL_TO_SERVICE_DICT,
    REEDS_SIM_INQUEUE_SUBJECT,
...
if os.getenv("DEPLOY_MODE") != "local":
    schedule.every(24).hours.do(
        process_logs,
        folder_path=os.getenv("LOG_FOLDER"),
        session=DB_SESSION(),
...
def monitor_time(func):
    async def inner_wrapper(self, request):
        ...
        end_time = time.time()
        logger.info(
            f"Time spent processing request at url {request.url} is {round(end_time-start_time,3)} seconds"
        )
        return ret_val
...
def rate_limit(func):
    async def inner_wrapper(self, request):
        ...
        if not throttle:
            logger.info(
                f"Request accepted for service {request.rel_url} for user {request['userData']['username'] if 'userData' in request else request.remote}"
            )
            ret_val = await func(self, request)
            return ret_val
        else:
            logger.error(
                f"Request denied for service {request.rel_url} for user {request['userData']['username'] if 'userData' in request else request.remote}"
            )
            return web.Response(
                text=json.dumps(
                    {
...
def authenticate(func):
    async def inner_wrapper(self, request):
        ...
        try:
            token = (
                request.headers["Authorization"].split(" ")[1].encode("utf-8")
            )
...
class Handler:
    def __init__(self, loop=None, pool=None):
        ...
        sim_thread = threading.Thread(
            ...
            target=run_simulation_using_queue,
            args=(
    ...
    async def handle_get_scenario_file_infos(self, request):
        ...
        try:
            ...
            custom_input = {
                "TechCost_file": get_file_category(
                    "TechCost_file", case_dict["TechCost_file"]
                ),
                "FuelLimit_file": get_file_category(
                    "FuelLimit_file", case_dict["FuelLimit_file"]
                ),
                "MinLoad_file": get_file_category(
                    "MinLoad_file", case_dict["MinLoad_file"]
                ),
            }

            ivt_base_dir = ivt_file = os.path.join(
                os.path.dirname(__file__),
                "..",
                "..",
                "A_Inputs",
            ...
            ivt_file = (
                os.path.join(ivt_base_dir, case_dict["IVT_file"])
                if case_dict["IVT_file"].endswith(".csv")
                else os.path.join(ivt_base_dir, "ivt.csv")
            )
    ...
    async def handle_remove_card_label(self, request):
        ...
        if label_data["type"] == "scenario":
            ...
            with session_manager() as session:
                self.db.delete_scenario_card_label(
                    session,
                    request["userData"]["username"],
                    label_data["scen_name"],
                    label_data["label"],
                )
                session.commit()
        ...
        try:
            ...
            if "attachement" in parsed_data:
                ...
                base_path = os.path.join(
                    os.path.dirname(__file__), "..", "bug_attachements"
                )

                with open(
                    os.path.join(
                        base_path, f"{parsed_data['attachement']['filename']}"
                    ),
                    "wb",
                ) as f:
    ...
    async def update_profile_data(self, request):
        ...
        try:
            ...
            if "avatar" in parsed_data:
                ...
                with session_manager() as session:
                    old_image_name = self.db.get_image_name(
                        session, parsed_data["formdata"]["username"]
                    )

                base_path = os.path.join(
                    os.path.dirname(__file__), "..", "users_avatar"
                )
                ...
                with open(
                    os.path.join(
                        base_path,
                        f"{parsed_data['formdata']['username']}.{parsed_data['avatar']['fileext']}",
                    ),
                    "wb",
                ) as f:
                ...
                parsed_data["formdata"][
                    "image_name"
                ] = f"{parsed_data['formdata']['username']}.{avatar['fileext']}"
    ...
    async def get_profile_data(self, request):
        ...
        try:
            with session_manager() as session:
                user_detail = self.db.get_user_data(
                    session, request["userData"]["username"]
                )
            if user_detail:
    ...
    async def get_profile_image(self, request):
        ...
        image_path = os.path.join(
            os.path.dirname(__file__), "..", "users_avatar", "default.png"
        )
        try:
            decoded = jwt.decode(token, JWT_KEY, algorithms=["HS256"])
            with session_manager() as session:
                image_name = self.db.get_image_name(
                    session, decoded["username"]
                )
            if image_name:
                image_path = os.path.join(
                    os.path.dirname(__file__), "..", "users_avatar", image_name
                )
        except Exception as e:
            pass
        print(image_path)
    ...
    async def update_password(self, request):
        ...
        try:
            ...
            with session_manager() as session:
                if self.hash.check_password(
                    session,
                    request["userData"]["username"],
                    user_detail["oldpassword"],
                ):

                    hash_key, salt = self.hash.hash_string(
                    ...
                    email_body = (
                        UserChangePasswordHTMLMessage().return_rendered_html({})
                    )
    ...
    async def handle_signup(self, request):
        ...
        try:
            ...
            with session_manager() as session:
                token_is_valid = self.db.is_signup_token_valid(
                    session, parsed_data["formdata"]["stoken"]
                )
            ...
            if token_is_valid:
                ...
                with session_manager() as session:
                    message, code = self.hash.signup(
                        session, parsed_data["formdata"]
                    )
                    self.db.invalidate_signup_token(session, stoken)
                    self.db.add_default_limits(
                        session, parsed_data["formdata"]["username"]
                    )
                    session.commit()
                message = "success"
            ...
            for key, subdict in parsed_data.items():
                ...
                if subdict["filename"].endswith(".csv"):
                    ...
                    if key in CSV_FILE_HEADERS:
                        df = pd.read_csv(
                            subdict["filename"], names=CSV_FILE_HEADERS[key]
                        )
                    else:
    ...
    async def get_file_to_download(self, request):
        ...
        try:
            ...
            decoded = jwt.decode(token, JWT_KEY, algorithms=["HS256"])
            file_type, file_category, scen_name, user = (
                decoded["file_type"],
                decoded["category"],
                decoded["scen_name"],
                decoded["user"],
            ...
            if file_name is None:
                ...
                if scen_name in list(case_df.columns):
                    ...
                    print(
                        f"No file exists for file type {file_type}, category {file_category}, scenario {scen_name} for user {request['userData']['username']}"
                    )
        ...
        try:
            ...
            if os.path.exists(os.path.join(output_folder_path, "exceloutput")):
                ...
                for file in os.listdir(
                    os.path.join(output_folder_path, "exceloutput")
                ):
                ...
                if file_exists:
                    if not os.path.exists(
                        os.path.join(output_folder_path, "exceloutput.zip")
                    ):
            ...
            if output_file:
                ...
                with open(
                    os.path.join(output_folder_path, output_file), "rb"
                ) as infile:
                    content = infile.read()
    ...
    async def get_list_of_scenarios(self, request):
        ...
        try:
            with session_manager() as session:
                ...
                personnel_scenarios = self.db.get_scenarios_by_user(
                    session, request["userData"]["username"]
                )
        ...
        try:
            ...
            with session_manager() as session:
                ...
                scenario_data["description"] = self.db.get_scenario_description(
                    session, username, scenario_name
                )
            ...
            model_year_file = (
                os.path.join(model_year_dir, scenario_data["yearset"])
                if "yearset" in scenario_data
                else os.path.join(model_year_dir, "modeledyears_set.csv")
            )
    ...
    async def run_reeds_model(self, request):
        ...
        try:
            ...
            reeds_input["run_name"] = (
                request["userData"]["username"] + "_" + reeds_input["run_name"]
            )
            metadata = {
                ...
                "uuid": reeds_uuid,
                "description": " + ".join(
                    [el for el in reeds_input["scenarios"]]
                ),
            }

            # Append case file path and output folder path
            reeds_input["case_csv"] = os.path.join(
                os.path.dirname(__file__),
                "..",
                "users_cases",
                request["userData"]["username"],
            ...
            reeds_input["output_folder_path"] = os.path.join(
                os.path.dirname(__file__),
                "..",
                "users_output",
                request["userData"]["username"],
            ...
            with session_manager() as session:
                ...
                self.db.insert_simulation_queue(
                    session,
                    reeds_input,
                    reeds_uuid,
                    request["userData"]["username"],
    ...
    async def delete_personnel_scenario_state(self, request):
        ...
        try:
            with session_manager() as session:
                self.db.delete_personnel_scenario_state(
                    session, request["userData"]["username"]
                )
                session.commit()
            return web.Response(
    ...
    async def get_personnel_scen_state(self, request):
        ...
        try:
            with session_manager() as session:
                data = self.db.get_personnel_scen_state(
                    session, request["userData"]["username"]
                )
    ...
    async def get_default_scen_state(self, request):
        ...
        try:
            with session_manager() as session:
                data = self.db.get_default_scen_state(
                    session, request["userData"]["username"]
                )
    ...
    async def handle_get_technologies(self, request):
        ...
        try:
            ...
            for i in range(len(df)):
                ...
                year_turned_on = [
                    key for key, val in row_data.items() if not np.isnan(val)
                ]
                ...
                technologies.append(
                    {
                        ...
                        if "BATTERY" not in tech_name
                        else "BATTERY (" + tech_name.split("_")[1] + " hour)",
    ...
    async def get_output_card_state(self, request):
        ...
        try:
            with session_manager() as session:
                data = self.db.get_output_card_state(
                    session, request["userData"]["username"]
                )
            if data:
                print(data.decode())
    ...
    async def update_personnel_scen_state(self, request):
        ...
        try:
            ...
            with session_manager() as session:
                self.db.update_personnel_scen_state(
                    session, request["userData"]["username"], binary_string
                )
                session.commit()
            return web.Response(
    ...
    async def update_default_scen_state(self, request):
        ...
        try:
            ...
            binary_string = data["data"].encode("utf-8")
            with session_manager() as session:
                self.db.update_default_scen_state(
                    session, request["userData"]["username"], binary_string
                )
                session.commit()
            return web.Response(
    ...
    async def update_output_card_state(self, request):
        ...
        try:
            ...
            binary_string = data["data"].encode("utf-8")
            with session_manager() as session:
                self.db.update_output_card_state(
                    session, request["userData"]["username"], binary_string
                )
                session.commit()
            return web.Response(
    ...
    async def clone_scenarios(self, request):
        ...
        try:
            ...
            if data["name"] not in list(case_df.columns):
                return web.Response(
                    text=json.dumps(
                        f"{data['name']} does not exist, can not clone"
                    ),
                    status=500,
                )
            ...
            if metadata["name"] in list(case_df.columns):
                return web.Response(
                    text=json.dumps(
                        f"{metadata['name']} already exists, please consider renaming ealier cloned copy and try again"
                    ),
                    status=500,
                )
                ...
                if isinstance(item, str):
                    if item.endswith(".csv") and key in FILE_KEYS:
                        ...
                        if item not in DEFAULT_FILE_TO_CATEGORY.get(key, {}):
                            ...
                            shutil.copy(
                                os.path.join(
                                    base_folder, FILE_LOCATION[key], item
                                ),
                                os.path.join(
                                    base_folder, FILE_LOCATION[key], file_name
                                ),
                            )
                            item = file_name
    ...
    async def update_scenarios(self, request):
        ...
        try:
            ...
            base_folder = os.path.join(
                os.path.dirname(__file__), "..", "..", "A_Inputs", "inputs"
            )
            form_data = parsed_data.pop("formdata")
            file_allow = parsed_data.pop("file_allow")
            technologies = parsed_data.pop("technologies")
            update_action = parsed_data.pop("action", {"update": False})[
                "update"
            ]

            metadata = {
                "name": form_data.pop("scen_name"),
                "author": request["userData"]["username"],
                "created": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),
                "status": "Ready to test",
            ...
            if update_action:
                ...
                case_row_dict = dict(
                    zip(case_rows, case_df[metadata["name"]].to_list())
                )
            else:
                # check if name already exists or if the name is empty
                if metadata["name"] in list(case_df.columns):
                ...
                if key in FILE_KEYS and file_allow[key] == "Custom Upload":
                    ...
                    if str(case_row_dict.get(key, None)).endswith(".csv"):
                        ...
                        try:
                            os.remove(
                                os.path.join(
                                    base_folder, FILE_LOCATION[key], prev_file
                                )
                            )
                        except FileNotFoundError as e:
        ...
        try:
            ...
            if data in scenarios_personnel:
                ...
                try:
                    ...
                    case_df = pd.read_csv(case_csv_file)
                    case_row_dict = dict(
                        zip(case_df["case"], case_df[data["name"]])
                    )
                    ...
                    for key, val in case_row_dict.items():
                        if key in FILE_KEYS:
                            ...
                            if val and not pd.isna(val):
                                if val not in DEFAULT_FILE_TO_CATEGORY.get(
                                    key, {}
                                ):
                                    try:
                                        os.remove(
                                        ...
                                        print(
                                            f"File not found for deleting! >> {str(e)}"
                                        )
    ...
    async def get_ouput_metadata(self, request):
        ...
        try:
            ...
            remove_uuids = []
            for reed_uuid in self.instances_dict.get(
                request["userData"]["username"], []
            ):

                run_name = self.instances_dict[request["userData"]["username"]][
                    reed_uuid
                ]["name"]
                ...
                if not self.instances_dict[request["userData"]["username"]][
                    reed_uuid
                ]["process"].is_alive():
    ...
    async def delete_output_metadata(self, request):
        ...
        try:
            ...
            with session_manager() as session:
                ...
                self.db.delete_simulation_queue(
    ...
    async def get_reset_token(self, request):
        ...
        with session_manager() as session:
            ...
            if user:
                ...
                if token != None:
                    reset_link = (
                        f"{os.getenv('BASE_URL_RESET_PASSWORD')}/{token}"
                    )

                    # Let's update the database
                    token = token.encode("utf-8")
                    self.db.insert_or_update_reset_token(
                        session, hashed_token, user.username
                    )
                    session.commit()

                    email_body = (
                        UserPasswordResetHTMLMessage().return_rendered_html(
    ...
    async def handle_password_reset(self, request):
        ...
        try:
            ...
            decoded = jwt.decode(token, JWT_KEY, algorithms=["HS256"])
            with session_manager() as session:
                validate_token = self.db.check_reset_token(
                    session, decoded["username"], token
                )
                if validate_token:

                    user_detail = await request.json()
                    hash_pwd, salt = self.hash.hash_string(
                        user_detail["password"]
                    )
                    self.db.update_user_info_by_username(
    ...
    async def handle_user_signup_request(self, request):
        ...
        try:
            ...
            email_body = (
                UserSignUpRequestNotificationHTMLMessage().return_rendered_html(
                    {
        ...
        try:
            ...
            with session_manager() as session:
                if self.db.is_admin(session, request["userData"]["username"]):
                    ...
                    user_requests = [
                        {
                            "name": req_obj.first_name
                            + " "
                            + req_obj.last_name,
                            "email": req_obj.email,
                            "request_date": req_obj.requested_date.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            ),
        ...
        try:
            ...
            with session_manager() as session:
                if self.db.is_admin(session, request["userData"]["username"]):
                    ...
                    self.db.update_signup_request_status(
                        session, data["email"], "APPROVED"
                    )
                    session.commit()
        ...
        try:
            ...
            with session_manager() as session:
                if self.db.is_admin(session, request["userData"]["username"]):
                    ...
                    self.db.update_signup_request_status(
                        session, data["email"], "REJECTED"
                    )
                    session.commit()

                    email_body = (
                        UserSignUpRejectionHTMLMessage().return_rendered_html(
                            {}
                        )
                    )
                    # print(email_body)
        ...
        try:
            with session_manager() as session:
                if self.db.is_admin(session, request["userData"]["username"]):
                    ...
                    # Let's create a token and send email out to user
                    self.db.update_bug_status(
                        session, data["uuid"], data["status"]
                    )
                    session.commit()
